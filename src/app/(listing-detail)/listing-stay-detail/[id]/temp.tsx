...
const ListingStayDetailPage: FC<ListingStayDetailPageProps> = () => {
  ...
  //! Fetching data of a particular property
  const [fetchedProperty, setFetchedProperty] = useState<Properties>();
  // const fetchProperties = async () => {
  //   console.log("fetching properties....");
  //   const response = await axios.get(`/api/particular/${param}`);
  //   console.log("Properties in Greece: ", response);
  //   setFetchedProperty(response?.data);
  // };

  const getParticular = async () => {
    const response = await Property.findById(param);
  ...
  const renderSection1 = () => {
    return (
      <div className="listingSection__wrap !space-y-6 border-4 border-blue-400">
        {/* 1 */}
